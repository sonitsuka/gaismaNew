...
import { getMusicSections, urlFor, isSanityConfigured } from "@/lib/sanity"

export const revalidate = 60 // Revalidate every 60 seconds

// Fallback data - original hardcoded content
const FALLBACK_MUSIC_SECTIONS = [
  {
    category: "Album & EP",
    name: "GAISMA",
    items: [
...
export default async function MusicPage() {
  ...
  const sanityMusicSections = await getMusicSections()

  // Use Sanity data if available, otherwise use fallback
  const musicSections = sanityMusicSections.length > 0 ? sanityMusicSections : FALLBACK_MUSIC_SECTIONS as any[]
